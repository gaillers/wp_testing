...
function template_scripts() {
	...
	wp_enqueue_style( 'template-opensans', 'https://fonts.googleapis.com/css2?family=Open+Sans:wght@300;400&display=swap" rel="styleshee' );
	wp_enqueue_style( 'template-slick-styles-css', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.css' );

	if ( !is_admin() ) {
	  wp_deregister_script( 'jquery' );
	  wp_register_script( 'jquery', ( 'https://code.jquery.com/jquery-3.5.1.min.js' ), false, null, true );
	  wp_enqueue_script( 'jquery' );
	  wp_enqueue_script( 'template-slick', 'https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js', array(), '20151215', true );
	  
	}

	wp_enqueue_style( 'template-main-style', get_template_directory_uri() . '/dist/main.css' );
	wp_enqueue_script( 'template-main-script', get_template_directory_uri() . '/dist/app.min.js', array(), false, true );
	wp_localize_script( 'template-main-script', 'wp', [
